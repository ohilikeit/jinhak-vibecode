...
#
# 격리된 FAKE_HOME에서 6 호스트 dir에 jinhak/ 네임스페이스 폴더가
...
[ ! -d "$FAKE_HOME/.claude/commands/jinhak" ] && ok "dry-run: 디렉터리 미생성" || ng "dry-run인데 디렉터리 생성됨"
...
declare -a HOST_PATHS=(
  "$FAKE_HOME/.claude/commands/jinhak"
  "$FAKE_HOME/.cursor/commands/jinhak"
  "$FAKE_HOME/.codex/prompts/jinhak"
  "$FAKE_HOME/.gemini/commands/jinhak"
  "$FAKE_HOME/.gemini/antigravity/commands/jinhak"
  "$FAKE_HOME/.config/opencode/commands/jinhak"
)
...
for p in \
  ...
  "$FAKE_HOME/.codex/prompts/jinhak" \
  "$FAKE_HOME/.gemini/antigravity/commands/jinhak" \
  "$FAKE_HOME/.config/opencode/commands/jinhak" ; do
  count=$(ls "$p"/*.md 2>/dev/null | wc -l)
  [ "$count" -eq 12 ] && ok "$p: 12개 .md" || ng "$p: $count개 (기대 12)"
done

# ── 4) Gemini는 .toml 12개 ───────────────────────────────────
gcount=$(ls "$FAKE_HOME/.gemini/commands/jinhak"/*.toml 2>/dev/null | wc -l)
[ "$gcount" -eq 12 ] && ok "Gemini: 12개 .toml" || ng "Gemini .toml: $gcount개"

# ── 5) 네임스페이스 활성화 검증 — init.md 본문이 동일한지 ─────
src=$(head -1 "$ROOT/commands/init.md")
dst=$(head -1 "$FAKE_HOME/.claude/commands/jinhak/init.md")
[ "$src" = "$dst" ] && ok "Claude /jinhak:init 첫 줄 일치" || ng "첫 줄 다름: src='$src' dst='$dst'"

# ── 6) --host 필터 ──────────────────────────────────────────
TMP2=$(mktemp -d)
run_capture env HOME="$TMP2" node "$INSTALL" register --host=claude
[ -d "$TMP2/.claude/commands/jinhak" ] && ok "--host=claude 단독 등록" || ng "--host 필터 동작 안 함"
[ ! -d "$TMP2/.cursor/commands/jinhak" ] && ok "--host=claude: 다른 호스트 미생성" || ng "다른 호스트가 의도치 않게 생성"
rm -rf "$TMP2"
